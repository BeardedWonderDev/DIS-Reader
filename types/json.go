...
func ParseJSONBody(logger *slog.Logger, body io.Reader, obj interface{}) error {
	...
	if reflectVal.Kind() != reflect.Pointer {
		logger.Error("Second argument to ParseJSONBody must be a reference")
	...
	return ValidateStruct(logger, obj)
}
	...
	if err != nil {
		...
		case validator.ValidationErrors:
			for _, err := range err {
				...
				if !fieldFound {
					logger.Debug("field not found on struct",
						slog.String("field", err.Field()),
						slog.String("type", fmt.Sprint(objType)),
					)
					return NewInvalidRequestError("Invalid request body")
				}
